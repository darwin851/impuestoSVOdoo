...
class Remesa_Sv(models.Model):
    ...
    total = fields.Float('Total Remesa',
                         ...
                         required=True,
                         readonly=True, help="Total depositado al banco")

    referencia = fields.Char('Referencia Banco', required=True,
                             readonly=True,
    ...
    def contabilizar_remesa(self):
        ...
        if not self.cuenta_id.default_account_id  or not self.origen_id.default_account_id:
            raise exceptions.except_orm(_('Cuenta No Selecccionada!'),
                                        _('Por Favor seleccionar una cuenta bancaria para poder registrar la remesa'))
        if not self.total:
            raise exceptions.except_orm(
                _('Total No Asignado!'),
    ...
    def move(self, journal_id, tipo):
        ...
        if tipo == 'a':
            ...
            account_credit_id = self.origen_id.default_account_id.id

        # cuenta destino
        if tipo == 'b':
            account_debit_id = self.cuenta_id.default_account_id.id
